...
class Word2VecSearch(ModelSearch):
    ...
    def __init__(self, train:bool, data_path:str, model_path:str = None, tfidf_prepro = False, vector_size:int = 300, 
        ...
        if super().train == True:
            ...
            self.model_load(super().model_path, super().data_path)
    ...
    def model_train(self, data_path:str):
        ...
        text.append([x for x in self.df_docs['title']])

        #Extrahuji texty ze seznam datasetu
        seznam_df = utils.load_seznam('X:/Data/BP/dareczech/dev.tsv')
        self.process_documents(seznam_df)
        text.append([x for x in seznam_df['doc']])
        text.append([x for x in seznam_df['title']])
        ...
        self.df_docs.to_csv(f'{os.path.dirname(super().data_path)}/docs_cleaned.csv', index=False)

        self.model_save('X:/Data/BP/w2v_model')
    ...
    def ranking_ir(self, query:str, n:int) -> pd.DataFrame:   
        query = super().prepro.process_sentence(query)
